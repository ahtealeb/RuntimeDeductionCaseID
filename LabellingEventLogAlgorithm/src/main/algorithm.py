...
class Algorithm:
    ...
    numOfCases = 0
    CasesBranches = dict()
    #To be added :: in case of outer loop on [build branch tree without inner loop] function 
    eventIdentifier=0               
    labelCaseId = 1
    ...
    '''Calculate probability of each activity within giving sequence'''
    def calculate_activity_probability(self, sequence):
        ...
        for a in appearances:
            if (sequnceLength!=0):
                ...
                print "Sequence of events is empty! ..."
                userInput = raw_input("press x for Exit and c to continue...")
                if (userInput=="x"):
                    return
                if (userInput=="c"):
                    pass
    ...
    def check_possible_branches_based_on_heuristics(self, symbol, possibleNodes):
        ...
        for n in possibleNodes:
            ...
            if(symbolTimestamp.isdigit()) :  
                ...
                try:
                    symbolTimestampDatetime=datetime.datetime.strptime(symbolTimestamp,'%m/%d/%Y  %I:%M:%S %p')#'%m/%d/%Y  %H:%M'
                    #datetime.datetime.strptime(node.timestampDatetime,'%m/%d/%Y  %I:%M:%S %p')
                    nodeDateTime=node.timestampDatetime  
    ...
    def build_branches_tree(self,symbol):
        #eventIdentifier=0               
        #labelCaseId = 1
        # to be removed :: instead of this loop send symbol as an input argument for function 
        ...
        #print "event id",self.eventIdentifier
        for n in dictionary :
            caseId = n.caseId
            if(caseId == 0):
            ...
            if(symbol[0].isdigit()):
                ...
                try:
                    ...
                    timestampDatetime=datetime.datetime.strptime(symbol[0],'%m/%d/%Y  %I:%M:%S %p')
                    #print "timestampDatetime ",timestampDatetime
                    #timestamp=timestampDatetime.hour
                    timestamp=self.eventIdentifier
                except ValueError:
                    try:
                        symbolTimestampDatetime=datetime.datetime.strptime(symbol[0],'%m/%d/%Y  %H:%M')
                        timestamp=self.eventIdentifier
                    except ValueError:
                        print  "error in parsing datetime from string "
                        print ValueError
            symbolNode = self.traces.add_node(percentage, 1.0 / numPossibleBranches,self.eventIdentifier ,timestamp, symbol[1], caseId, n,timestampDatetime)
            
            # list of created nodes of this symbol
            if(not symbolNodesDic.has_key(caseId)):
                symbolNodesDic[caseId]=0
            symbolNodesDic[caseId]=symbolNodesDic[caseId]+symbolNode.percentage
    ...
    def calculate_confidence_level_branch(self):
        ...
        # used event  log to decrease the size of event log
        isDicClean = False
        duplicateEL = []
    ...
    def apply_algorithm(self, symbol,dashboard_filename):
        #print "build cases tree"
        
        # To read Batch, input example 
        #for symbol in self.S:        
        caseNodesDic=self.build_branches_tree(symbol)
        f = open(dashboard_filename,'ab')
        w = csv.writer(f, delimiter = ',')
        currentTime = datetime.datetime.now().strftime('%m/%d/%Y  %I:%M:%S %p')
            
        if(len(caseNodesDic)==0):# to detect out of range
            print symbol[0],"; ",symbol[1],"is unassigned with unknown percentage"
            caseIdValue = "Unassigned"
            rsValue = "Unknown"
            w.writerow([currentTime,caseIdValue,symbol[0],symbol[1],rsValue])
        for caseId in caseNodesDic:
            print "case id; ",caseId," ; ",symbol[0],"; ",symbol[1],"; percentage ",caseNodesDic[caseId]
            #write on file for further analysis (as dashboard in CSV file)
            #toWrite = currentTime+";"+str(caseId)+";"+symbol[0]+";"+symbol[1]+";"+str(caseNodesDic[caseId])
            w.writerow([currentTime,str(caseId),symbol[0],symbol[1],str(caseNodesDic[caseId])])
        f.close()
            
        self.numOfCases = len(self.root.children)
        
    def finish_algorithm(self, start_time_build_tree):
...
''' # old: running batch of S all at once  
