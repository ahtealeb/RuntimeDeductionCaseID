...
class Algorithm:
    ...
    def calculate_confidence_level_branch(self):
        ...
        #self.CasesBranches[0]=[].append(self.branchRoot)
        branchId=1
        for leaf in self.tracesLeafs:#self.traces.get_leafs(self.root):
            nodes = []
            ...
            #tempCaseBranches=self.CasesBranches[branch.caseId-1]
            for branchLeaf in caseBranches:#tempCaseBranches:
            ...
            for b in possibleBranches:
                branchNode=self.traceLogsTree.add_branch_object(branch, b)
                if(not self.CasesBranches.has_key(branch.caseId)):
                    self.CasesBranches[branch.caseId]=[]
                self.CasesBranches[branch.caseId].append(branchNode)
                if(b in self.eventLogsLeafs):
                    self.eventLogsLeafs.remove(b);
                if(branchNode not in self.eventLogsLeafs):
                    self.eventLogsLeafs.append(branchNode)
    ...
    def build_traceLog(self):
        traceLogId=1
        for branchLeaf in self.eventLogsLeafs:#self.traceLogsTree.get_leafs(self.branchRoot):
            branches = []
